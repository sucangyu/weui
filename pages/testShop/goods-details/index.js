...
Page({
  ...
  goShopCar: function () {
    wx.reLaunch({
      url: "/pages/testShop/shop-cart/index"
    });
  },
  ...
  */
    ...
    if (shoptype == 'toPingtuan') {
      wx.request({
        ...
        success: function (res) {
          ...
          wx.navigateTo({
            url: "/pages/testShop/to-pay-order/index?orderType=buyNow&pingtuanOpenId=" + res.data.data.id
          }) 
        }
      })      
    } else {
      wx.navigateTo({
        url: "/pages/testShop/to-pay-order/index?orderType=buyNow"
      }) 
    }
  ...
  onShareAppMessage: function () {
    return {
      ...
      path: '/pages/testShop/goods-details/index?id=' + this.data.goodsDetail.basicInfo.id + '&inviter_id=' + wx.getStorageSync('uid'),
      success: function (res) {
        // 转发成功
      },
      fail: function (res) {
    ...
    wx.request({
      ...
      success: function (res) {
        if (res.data.code == 0) {
          ...
          wx.navigateTo({
            url: "/pages/testShop/kanjia/index?kjId=" + res.data.data.kjId + "&joiner=" + res.data.data.uid + "&id=" + res.data.data.goodsId
          })
        } else {
          wx.showModal({
            title: '错误',
  ...
  joinPingtuan: function (e) {
    ...
    wx.navigateTo({
      url: "/pages/testShop/to-pay-order/index?orderType=buyNow&pingtuanOpenId=" + pingtuanopenid
    }) 
  }
})
